...
class admin_editUserController {
	...
	public function checkId()
	{
		if (!empty($_GET['id'])) {
			...
			if (empty($this->data)) {
				header('Location: ' . $this->router->generate('indexUsers'));
				die;
			}
		}
	}
	...
	private function saveMovie()
	{
		if (!empty($_POST) && !$this->formError) {			
			...
			if ($request->execute($data)) {
				...
				header('Location: ' . $this->router->generate('updateUsers', ['id' => $idRedirect]));
				die;
			}
			else {
...
$ctrl = new admin_editUserController($db, $router);
